...
const entries = document.querySelectorAll("div.row > div.col-md-4 > div");
for (let i = 0; i < entries.length; i++) {
    const imageUrl = entries[i].children[0].href;
    const countryName = entries[i].children[1].innerHTML;
    console.log(`
    "${countryName}": {
        alternateNames: [],
        imageUrl: "${imageUrl}"
...
/*
Big mapping from all possible sanitized names to the "offical" name that is
a key in `flags`. Code to generate:

possibleNameToOfficalName = {}
for (countryName in flags) {
    let possibleNames = [];
    possibleNames.push(countryName);
    possibleNames.push.apply(possibleNames, flags[countryName].alternateNames);
    possibleNames = possibleNames.map(n => standardizeString(n));
    possibleNames.forEach(n => possibleNameToOfficalName[n] = countryName);
}
console.log(possibleNameToOfficalName);
